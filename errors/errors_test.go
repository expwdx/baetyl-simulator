...
func TestErrors(t *testing.T) {
	...
	assert.Equal(t, e, Cause(e))
	assert.EqualError(t, e, "abc")
	assert.Equal(t, e.(Coder).Code(), "c")
	assert.Contains(t, fmt.Sprintf("%+v", e), "baetyl-go/errors/error.go")
	...
	assert.Equal(t, e3, Cause(e3))
	assert.Equal(t, e4, Cause(e4))
	assert.Contains(t, fmt.Sprintf("%+v", e3), "baetyl-go/errors/error.go")
	assert.Contains(t, fmt.Sprintf("%+v", e4), "baetyl-go/errors/error.go")
